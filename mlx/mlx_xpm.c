...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		if (!(line = f(info,&pos,info_size)) ||
						!(tab = mlx_int_str_to_wordtab(line)) || !( W = atoi(tab[0])) ||
		...
		while (i--)
		{
				...
				{
						if (!(clip_data = malloc(4* W*height)) ||   ok, nice size ..
										!(clip_img = XCreateImage(xvar->display, xvar->visual,
														1, XYPixmap, 0, clip_data,
														 W, height, 8, ( W+7)/8)) )
								RETURN;
						memset(clip_data, 0xFF, 4* W*height);
				}
				*/
				if (method)
		...
		if (!(img = mlx_new_image(xvar, W,height)))
				RETURN;
		opp = img->bpp/8;
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int * W,int *height)
{
		int	fd;
		int	size;
		char	*ptr;
...
void	*mlx_xpm_to_image(t_xvar *xvar,char **xpm_data,int * W,int *height)
{
		t_img	*img;
