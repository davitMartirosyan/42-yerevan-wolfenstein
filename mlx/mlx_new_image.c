...
/*
**  Data malloc :   W+32 ( bitmap_pad=32 ),    *4 = *32 / 8bit
*/


void	*mlx_int_new_xshm_image(t_xvar *xvar,int  W,int height,int format)
{
  t_img	*img;
  int	(*save_handler)();
  ...
  img->image = XShmCreateImage(xvar->display,xvar->visual,xvar->depth,
			       format,img->data,&(img->shm), W,height);
  if (!img->image)
    {
      free(img);
      return ((void *)0);
  ...
  img->format = format;
  img->shm.shmid = shmget(IPC_PRIVATE,( W+32)*height*4,IPC_CREAT|0777);
  if (img->shm.shmid==-1)
    {
...
void	*mlx_int_new_image(t_xvar *xvar,int  W, int height,int format)
{
  t_img	*img;

  if (!(img = malloc(sizeof(*img))))
    return ((void *)0);
  if (!(img->data = malloc(( W+32)*height*4)))
  {
    free(img);
    return ((void *)0);
  }
  bzero(img->data,( W+32)*height*4);
  img->image = XCreateImage(xvar->display,xvar->visual,xvar->depth,format,0,
			    img->data, W,height,32,0);
  if (!img->image)
    {
      free(img->data);
      free(img);
  ...
  img-> W =  W;
  img->height = height;
  img->pix = XCreatePixmap(xvar->display,xvar->root, W,height,xvar->depth);
  img->format = format;
...
void	*mlx_new_image(t_xvar *xvar,int  W, int height)
{
  t_img	*img;

  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar, W,height,ZPixmap))
      return (img);
  return (mlx_int_new_image(xvar, W,height,ZPixmap));
}

void	*mlx_new_image2(t_xvar *xvar,int  W, int height)
{
  t_img	*img;

  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar, W,height,XYPixmap))
      return (img);
  return (mlx_int_new_image(xvar, W,height,XYPixmap));
}
